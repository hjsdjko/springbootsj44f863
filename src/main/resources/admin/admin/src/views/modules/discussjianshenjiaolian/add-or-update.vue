	...
	<div class="addEdit-block" :style='{"padding":"30px"}'>
		...
		>
			<template >
				...
				<el-form-item :style='{"width":"48%","margin":"0 0 20px 0"}' v-if="type!='info'"  label="评论内容" prop="content">
					<editor 
				...
				<el-form-item :style='{"width":"48%","margin":"0 0 20px 0"}' v-else-if="ruleForm.content" label="评论内容" prop="content">
				    <span :style='{"fontSize":"14px","lineHeight":"40px","color":"#333","fontWeight":"500","display":"inline-block"}' v-html="ruleForm.content"></span>
				</el-form-item>
				<el-form-item :style='{"width":"48%","margin":"0 0 20px 0"}' v-if="type!='info'"  label="回复内容" prop="reply">
					<editor 
						style="min-width: 200px; max-width: 600px;"
				...
				<el-form-item :style='{"width":"48%","margin":"0 0 20px 0"}' v-else-if="ruleForm.reply" label="回复内容" prop="reply">
...
export default {
    ...
    info(id) {
      this.$http({
        url: `discussjianshenjiaolian/info/${id}`,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
				...
				this.$http({ 
					...
					if (data && data.code === 0) { 
						if(data.data.total>=crossoptnum) {
							...
							this.$http({
								url: `discussjianshenjiaolian/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.discussjianshenjiaolianCrossAddOrUpdateFlag = false;
											this.parent.search();
