...
<div :style='{"width":"80%","padding":"20px","margin":"10px auto","position":"relative","background":"#fff"}'>
    ...
    >
          ...
          <el-form-item :style='{"width":"49%","padding":"10px","margin":"0 0 10px","background":"none"}' label="密码" prop="mima">
          ...
          <el-form-item :style='{"width":"49%","padding":"10px","margin":"0 0 10px","background":"none"}' label="年龄" prop="nianling">
            <el-input v-model="ruleForm.nianling" 
                placeholder="年龄" clearable :disabled=" false  ||ro.nianling"></el-input>
          </el-form-item>
          <el-form-item :style='{"width":"49%","padding":"10px","margin":"0 0 10px","background":"none"}' label="身高" prop="shengao">
            <el-input v-model="ruleForm.shengao" 
                placeholder="身高" clearable :disabled=" false  ||ro.shengao"></el-input>
          </el-form-item>
          <el-form-item :style='{"width":"49%","padding":"10px","margin":"0 0 10px","background":"none"}' label="体重" prop="tizhong">
          ...
          <el-form-item :style='{"width":"49%","padding":"10px","margin":"0 0 10px","background":"none"}' label="职业履历" prop="zhiyelvli">
            <editor 
...
<script>
  export default {
    ...
    // 多级联动参数
      // 多级联动参数
      info() {
        this.$http.get(`jianshenjiaolian/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
			...
			this.$refs["ruleForm"].validate(valid => {
				if(valid) {
					...
					if(crossrefid && crossuserid) {
						...
						this.$http.get('jianshenjiaolian/list', {
							params: params
						}).then(res => {
							if(res.data.data.total>=crossoptnum) {
								...
								this.$http.post(`jianshenjiaolian/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(res => {
									if (res.data.code == 0) {
										this.$message({
											message: '操作成功',
											type: 'success',
