...
<div>
	...
	<div class="list-preview" :style='{"width":"100%","padding":"0","margin":"10px auto","position":"relative","background":"none"}'>
		...
		<div class="category-3" :style='{"padding":"5px","margin":"-24px auto 0 calc(10% + 38px)","background":"url(http://codegen.caihongy.cn/20240120/777684ebe3104d3cb040057bea6e001c.png) center center/100% 100% no-repeat","display":"flex","width":"80%","justifyContent":"center","height":"60px"}'>
			...
			<div class="item" :class="swiperIndex == index ? 'active' : ''" v-for="(item, index) in fenlei" :key="index" @click="getList(1, item[feileiColumn], 'btn' + index)" :ref="'btn' + index" plain>
				<img v-if="item.image" :style='{"width":"40px","margin":"0 6px 0 0","objectFit":"cover","display":"block","height":"40px"}' :src="baseUrl + (item.image?item.image.split(',')[0]:'')">
    ...
    <el-form :inline="true" :model="formSearch" class="list-form-pv" :style='{"padding":"10px","boxShadow":"0 4px 10px rgba(0,0,0,.3)","margin":"10px auto","alignItems":"center","borderRadius":"30px","flexWrap":"wrap","background":"#fff","display":"flex","width":"83%","height":"auto"}'>
      <el-form-item :style='{"border":"1px solid #000","margin":"0","flex":"1","borderWidth":"0 0px 0 0"}'>
	    <div class="lable" v-if="false" :style='{"width":"auto","padding":"0 10px","lineHeight":"42px","display":"inline-block"}'>教练工号：</div>
        <el-input v-model="formSearch.jiaoliangonghao" placeholder="教练工号" @keydown.enter.native="getList(1, curFenlei)" clearable></el-input>
      </el-form-item>
      <el-form-item :style='{"border":"1px solid #000","margin":"0","flex":"1","borderWidth":"0 0px 0 0"}'>
	    <div class="lable" v-if="false" :style='{"width":"auto","padding":"0 10px","lineHeight":"42px","display":"inline-block"}'>教练姓名：</div>
        <el-input v-model="formSearch.jiaolianxingming" placeholder="教练姓名" @keydown.enter.native="getList(1, curFenlei)" clearable></el-input>
      </el-form-item>
	  ...
	  <el-button v-if="btnAuth('jianshenjiaolian','新增')" :style='{"cursor":"pointer","border":"0","padding":"0px 15px","margin":"0 10px 0 0","outline":"none","color":"#fff","borderRadius":"30px","background":"#3554A4","width":"100px","fontSize":"14px","lineHeight":"42px","height":"42px"}' type="primary" @click="add('/index/jianshenjiaolianAdd')"><i v-if="true" :style='{"color":"#fff","margin":"0 10px 0 0","fontSize":"14px"}' class="el-icon-circle-plus-outline"></i>添加</el-button>
    </el-form>
	...
	<div class="list" :style='{"width":"100%","padding":"30px 10% 230px","margin":"0px 0 10px","background":"url(http://codegen.caihongy.cn/20240219/48efb4be31ee443597196e704f0766c2.png) center bottom / 100% auto no-repeat,url(http://codegen.caihongy.cn/20240219/e541597a8bf34224a765175ed4cb8569.png) repeat-y center top / 100% auto"}'>
		...
		<div class="list2 index-pv1" :style='{"padding":"0px","flexWrap":"wrap","background":"none","display":"flex","width":"100%","justifyContent":"space-between","height":"auto"}'>
			<div :style='{"border":"2px solid #fff","padding":"20px","boxShadow":"0 4px 10px rgba(0,0,0,.3)","margin":"0 0 30px","alignItems":"center","color":"#000","background":"#fff","display":"flex","width":"49%","fontSize":"0","position":"relative","height":"auto"}' v-for="(item, index) in dataList" :key="index" @click.stop="toDetail(item)" class="list-item animation-box">
				<div :style='{"border":"0","width":"45%","padding":"0","overflow":"hidden","height":"240px"}'>
					<img @click.stop="imgPreView(item.zhaopian)" :style='{"width":"100%","objectFit":"cover","display":"block","height":"100%"}' v-if="item.zhaopian && item.zhaopian.substr(0,4)=='http'" :src="item.zhaopian.split(',')[0]" class="image" />
					<img @click.stop="imgPreView(baseUrl + (item.zhaopian?item.zhaopian.split(',')[0]:''))" :style='{"width":"100%","objectFit":"cover","display":"block","height":"100%"}' v-else :src="baseUrl + (item.zhaopian?item.zhaopian.split(',')[0]:'')" class="image" />
				</div>
				<div class="item-info" :style='{"padding":"10px","overflow":"hidden","flexWrap":"wrap","flex":"1","display":"flex","height":"auto"}'>
					<div :style='{"padding":"0 10px","color":"inherit","width":"100%","lineHeight":"50px","fontSize":"20px","fontWeight":"bold","order":"1"}' class="name ">{{item.jiaolianxingming}}</div>
					...
					<div :style='{"padding":"0 10px","order":"3"}'>
					  <span class="icon iconfont icon-shoucang10" :style='{"margin":"0 5px 0 0","lineHeight":"30px","fontSize":"14px","color":"inherit"}'></span>
					  <span :style='{"color":"inherit","lineHeight":"30px","fontSize":"14px"}'>{{item.storeupnum}}</span>
					</div>
				</div>
			</div>
		</div>
...
<script>
  export default {
    ...
    methods: {
      ...
      getFenlei() {
		this.$http.get('kechengmingcheng/list',{}).then(res => {
		  if (res.data.code == 0) {
		    this.fenlei = res.data.data.list
		  }
		});
		this.feileiColumn = 'jianshenkecheng'
      },
      getList(page, fenlei, ref = '') {
        ...
        let searchWhere = {};
        if (this.formSearch.jiaoliangonghao != '') searchWhere.jiaoliangonghao = '%' + this.formSearch.jiaoliangonghao + '%';
        if (this.formSearch.jiaolianxingming != '') searchWhere.jiaolianxingming = '%' + this.formSearch.jiaolianxingming + '%';
        if (this.formSearch.xingbie != '') searchWhere.xingbie = this.formSearch.xingbie;
        if (this.curFenlei != '全部') searchWhere.jianshenkecheng = this.curFenlei;
		if (this.sortType) searchWhere.sort = this.sortType
		if (this.sortOrder) searchWhere.order = this.sortOrder
        this.$http.get(`jianshenjiaolian/${this.centerType?'page':'list'}`, {params: Object.assign(params, searchWhere)}).then(res => {
          if (res.data.code == 0) {
      ...
      toDetail(item) {
        ...
        this.$router.push({path: '/index/jianshenjiaolianDetail', query: params});
      },
