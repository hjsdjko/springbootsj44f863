...
<div>
	<div :style='{"padding":"20px 20px 20px 15%","margin":"10px auto 0","borderRadius":"27px","background":"url(http://codegen.caihongy.cn/20240120/d40775f482fb4eaa8a715212bf1ab10f.png)","width":"80%","backgroundSize":"100% 100%","backgroundRepeat":"no-repeat"}' class="breadcrumb-preview">
		<el-breadcrumb :separator="'Ξ'" :style='{"fontSize":"14px","lineHeight":"1"}'>
			...
			<el-breadcrumb-item class="item2" v-for="(item, index) in breadcrumbItem" :key="index" to="/index/qicaileixing"><a>{{item.name}}</a></el-breadcrumb-item>
		...
		<div class="attr" :style='{"minHeight":"75px","padding":"0","background":"none","display":"flex","width":"55%","position":"relative","order":"2"}'>
			...
			<div class="info" :style='{"padding":"10px","margin":"0 0 0 0","background":"#fff","flex":"1"}'>
				...
				<div class="btn" :style='{"padding":"10px 0","flexWrap":"wrap","display":"flex"}'>
					<el-button :style='{"border":"0","cursor":"pointer","padding":"0 10px","margin":"0 auto 10px","outline":"none","color":"#fff","borderRadius":"10px","background":"#3554A4","width":"40%","lineHeight":"40px","fontSize":"14px","height":"40px"}' v-if="btnAuth('qicaileixing','修改')" @click="editClick">修改</el-button>
					<el-button :style='{"border":"0","cursor":"pointer","padding":"0 10px","margin":"0 auto 10px","outline":"none","color":"#000","borderRadius":"10px","background":"#FFC174","width":"40%","lineHeight":"40px","fontSize":"14px","height":"40px"}' v-if="btnAuth('qicaileixing','删除')" @click="delClick">删除</el-button>
					<!-- hasChat 无 -->
				</div>
			</div>
		</div>
  ...
  export default {
    ...
    methods: {
      ...
      nextClick(page) {
		...
		// 修改
		editClick(){
			this.$router.push(`/index/qicaileixingAdd?type=edit&&id=${this.detail.id}`);
		},
		// 删除
		delClick(){
			this.$confirm('是否删除此器材类型？')
			  .then(_ => {
			    this.$http.post('qicaileixing/delete', [this.detail.id]).then(res => {
			      if (res.data.code == 0) {
			        this.$message({
			          type: 'success',
			          message: '删除成功!',
