  ...
  export default {
    data() {
      ...
      return {
        ...
        rules: {
          jianshenkecheng: [
            { required: true, message: '健身课程不能为空', trigger: 'blur' },
          ],
        },
		centerType: false,
      };
    ...
    // 多级联动参数
      // 多级联动参数
      info() {
        this.$http.get(`kechengmingcheng/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
			...
			this.$refs["ruleForm"].validate(valid => {
				if(valid) {
					...
					if(crossrefid && crossuserid) {
						...
						this.$http.get('kechengmingcheng/list', {
							params: params
						}).then(res => {
							if(res.data.data.total>=crossoptnum) {
								...
								this.$http.post(`kechengmingcheng/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(res => {
									if (res.data.code == 0) {
										this.$message({
											message: '操作成功',
											type: 'success',
