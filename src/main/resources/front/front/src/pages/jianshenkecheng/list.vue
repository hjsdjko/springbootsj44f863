...
<div>
    ...
    <el-form :inline="true" :model="formSearch" class="list-form-pv" :style='{"padding":"10px","boxShadow":"0 4px 10px rgba(0,0,0,.3)","margin":"10px auto","alignItems":"center","borderRadius":"30px","flexWrap":"wrap","background":"#fff","display":"flex","width":"83%","height":"auto"}'>
	  ...
	  <el-button v-if="btnAuth('jianshenkecheng','新增')" :style='{"cursor":"pointer","border":"0","padding":"0px 15px","margin":"0 10px 0 0","outline":"none","color":"#fff","borderRadius":"30px","background":"#3554A4","width":"100px","fontSize":"14px","lineHeight":"42px","height":"42px"}' type="primary" @click="add('/index/jianshenkechengAdd')"><i v-if="true" :style='{"color":"#fff","margin":"0 10px 0 0","fontSize":"14px"}' class="el-icon-circle-plus-outline"></i>添加</el-button>
    </el-form>
...
<script>
  export default {
    ...
    created() {
		...
		this.$http.get('option/kechengmingcheng/jianshenkecheng').then(res => {
			if (res.data.code == 0) {
				this.jianshenkechengOptions = res.data.data;
				this.selectOptionsList.push({name:'健身课程',list:this.jianshenkechengOptions,tableName: 'jianshenkecheng',check: -1})
			}
		});
      this.getFenlei();
      this.getList(1, '全部');
    ...
    methods: {
      ...
      getList(page, fenlei, ref = '') {
        ...
        this.$http.get(`jianshenkecheng/${this.centerType?'page':'list'}`, {params: Object.assign(params, searchWhere)}).then(res => {
          if (res.data.code == 0) {
      ...
      toDetail(item) {
        ...
        this.$router.push({path: '/index/jianshenkechengDetail', query: params});
      },
